...
def main():
    ...
    for entry_key, data_seq in tqdm(items):
        ...
        ######################## Smampling texture ########################
        batch_size = 1
        uv_sampler = uv_sampler.repeat(
            batch_size, 1, 1, 1
        )  ##torch.Size([B, 13776, 4, 2])
        ...
        textures = torch.nn.functional.grid_sample(
            texture_image[None,], uv_sampler, align_corners=True
        )  # torch.Size([N, 3, 13776, 4])
    ...
    while True:
        ...
        if recording:
            ...
            w, h, _ = rgb.shape
            w, h = math.floor(w / 2.0) * 2, math.floor(h / 2.0) * 2
            rgb = rgb[:w, :h, :]
