...
if __name__ == "__main__":
    ...
    dataset_path = (
        "/hdd/zen/dev/meta/EgoQuest/data/amass/pkls/amass_isaac_run_upright_slim.pkl"
    )
    ...
    pkl_files = glob.glob(
        f"output/HumanoidIm/{exp_name}/phc_act/{motion_file_name}/*.pkl"
    )
    ...
    with h5py.File(
        f"output/HumanoidIm/{exp_name}/phc_act/phc_act_{motion_file_name}.h5", "w"
    ) as hdf5_file:
        metadata_dump = {}
        for key, value in full_dataset.items():
            ...
            if key in ["obs", "env_action", "reset", "clean_action", "reset"]:
                h5_dataset = hdf5_file.create_dataset(
        ...
        joblib.dump(
            metadata_dump,
            f"output/HumanoidIm/{exp_name}/phc_act/phc_act_{motion_file_name}_metadata.pkl",
            compress=True,
        )
    ...
    joblib.dump(
        full_dataset,
        f"output/HumanoidIm/{exp_name}/phc_act/phc_act_{motion_file_name}.pkl",
        compress=True,
    )
