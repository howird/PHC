...
if __name__ == "__main__":
    ...
    print(
        f"PNN Processing for: exp_name: {exp_name}, idx: {trained_idx}, epoch: {epoch}"
    )
    import ipdb
    ...
    checkpoint = torch_ext.load_checkpoint(
        f"output/HumanoidIm/{exp_name}/Humanoid_{epoch:08d}.pth"
    )
    ...
    loading_keys = [
        ...
        if k.startswith(f"{network_name_prefix}.{trained_idx}")
    ]
    copy_keys = [
        k
        for k in checkpoint["model"].keys()
        if k.startswith(f"{network_name_prefix}.{trained_idx + 1}")
    ]

    for idx, key_name in enumerate(copy_keys):
        checkpoint["model"][key_name].copy_(checkpoint["model"][loading_keys[idx]])

    torch_ext.save_checkpoint(
        f"output/HumanoidIm/{exp_name}/Humanoid_{epoch + 1:08d}", checkpoint
    )

    failed_dump = {
        key: amass_train_data_take6[key]
