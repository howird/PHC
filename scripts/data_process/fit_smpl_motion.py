...
def process_motion(key_names, key_name_to_pkls, cfg):
    ...
    shape_new, scale = joblib.load(
        f"data/{cfg.robot.humanoid_type}/shape_optimized_v1.pkl"
    )  # TODO: run fit_smple_shape to get this
    ...
    for data_key in pbar:
        ...
        for iteration in range(cfg.get("fitting_iterations", 500)):
            ...
            pbar.set_description_str(
                f"{data_key}-Iter: {iteration} \t {loss.item() * 1000:.3f}"
            )
...
def main(cfg: DictConfig) -> None:
    ...
    key_names = [
        "0-" + "_".join(data_path.split("/")[split_len:]).replace(".npz", "")
        for data_path in all_pkls
    ]
