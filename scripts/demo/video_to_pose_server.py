...
def download_model(model_type):
    ...
    model_zippath = tf.keras.utils.get_file(
        origin=f"{server_prefix}/{model_type}.zip", extract=True, cache_subdir="models"
    )
...
def frames_from_webcam():
    ...
    while cap.isOpened():
        ...
        frame = cv2.cvtColor(
            frame_orig, cv2.COLOR_BGR2RGB
        )  # send to the detector & model
        yolo_output = det_model.predict(
            source=frame, show=False, classes=[0], verbose=False
        )
        ...
        if len(yolo_output[0].boxes) > 0:
            ...
            bbox = np.stack(
                [
                    yolo_out_xyxy[:, 0],
                    yolo_out_xyxy[:, 1],
                    (yolo_out_xyxy[:, 2] - yolo_out_xyxy[:, 0]),
                    (yolo_out_xyxy[:, 3] - yolo_out_xyxy[:, 1]),
                ],
                axis=1,
            )
        ...
        if j2d is not None:
            for pt in j2d.reshape(-1, 2):
                ...
                frame_orig = cv2.circle(
                    frame_orig, (int(x), int(y)), 3, (255, 136, 132), 3
                )
...
def write_frames_to_video(
    ...
    print(
        f"######################## Writing number of frames {len(frames)} ########################"
    )
    if len(frames) == 0:
        return
    y_shape, x_shape, _ = frames[0].shape
    out = cv2.VideoWriter(
        out_file_name, cv2.VideoWriter_fourcc(*"FMP4"), frame_rate, (x_shape, y_shape)
    )
...
print("=================================================================")
print(
    "r: reset offset (use r:0.91), s: start recording, e: end recording, w: write video"
)
print("=================================================================")
