...
class AMPAgent(common_agent.CommonAgent):
    def __init__(self, base_name, config):
        ...
        if self.normalize_value:
            self.value_mean_std = RunningMeanStd((1,)).to(
                self.ppo_device
            )  # Override and get new value
    ...
    def set_stats_weights(self, weights):
        if self.normalize_input:
            if (
                weights["running_mean_std"]["running_mean"].shape
                == self.running_mean_std.state_dict()["running_mean"].shape
            ):
        ...
        if self._normalize_amp_input:
            if (
                weights["amp_input_mean_std"]["running_mean"].shape
                == self._amp_input_mean_std.state_dict()["running_mean"].shape
            ):
    ...
    def play_steps_rnn(self):
        ...
        for n in range(self.horizon_length):
            ...
            )  # this should upate mb_rnn_states
            if full_tensor:
                break

            if self.has_central_value:
                self.central_value_net.pre_step_rnn(
                    self.last_rnn_indices, self.last_state_indices
                )
            ...
            next_vals = self._eval_critic(
                input_dict
            )  # ZL this has issues? (maybe not, since we are passing the states in.)
    ...
    def pre_epoch(self, epoch_num):
        ...
        if self.vec_env.env.task.humanoid_type in ["smpl", "smplh", "smplx"]:
            ...
            if (
                (epoch_num > 1)
                and epoch_num % humanoid_env.shape_resampling_interval == 1
            ):  # + 1 to evade the evaluations.
            ...
            if humanoid_env.getup_schedule:
                humanoid_env.update_getup_schedule(
                    epoch_num, getup_udpate_epoch=humanoid_env.getup_udpate_epoch
                )
        ...
        self.running_mean_std_temp = copy.deepcopy(
    ...
    def _preproc_obs(self, obs_batch, use_temp=False):
        ...
        if self.normalize_input:
            ...
            if use_temp:
                ...
                obs_batch_orig = self.running_mean_std(
                    obs_batch_proc
                )  # running through mean std, but do not use its value. use temp
            else:
                obs_batch_out = self.running_mean_std(
                    obs_batch_proc
                )  # running through mean std, but do not use its value. use temp
            obs_batch_out = torch.cat(
    ...
    def calc_gradients(self, input_dict):
        ...
        obs_batch_processed = self._preproc_obs(
            obs_batch, use_temp=self.temp_running_mean
        )
        ...
        with torch.cuda.amp.autocast(enabled=self.mixed_precision):
            ...
            if rnn_masks is not None:
                ...
                (
                    ...
                    old_action_log_probs_batch[rnn_mask_bool],
                    action_log_probs[rnn_mask_bool],
                    advantage[rnn_mask_bool],
                    values[rnn_mask_bool],
                    entropy[rnn_mask_bool],
                    mu[rnn_mask_bool],
                    sigma[rnn_mask_bool],
                    return_batch[rnn_mask_bool],
                    old_mu_batch[rnn_mask_bool],
                    old_sigma_batch[rnn_mask_bool],
                )
    ...
    def _build_amp_buffers(self):
        ...
        )  # Demo is the data from the dataset. Real samples
    ...
    def _assemble_train_info(self, train_info, frame):
            ...
            train_info_dict.update(
                {
                    ...
                    "disc/agent_acc": torch_ext.mean_list(
                        train_info["disc_agent_acc"]
                    ).item(),
                    "disc/demo_acc": torch_ext.mean_list(
                        train_info["disc_demo_acc"]
                    ).item(),
                    "disc/agent_logit": torch_ext.mean_list(
                        train_info["disc_agent_logit"]
                    ).item(),
                    "disc/demo_logit": torch_ext.mean_list(
                        train_info["disc_demo_logit"]
                    ).item(),
                    "disc/grad_penalty": torch_ext.mean_list(
                        train_info["disc_grad_penalty"]
                    ).item(),
                    "disc/logit_loss": torch_ext.mean_list(
                        train_info["disc_logit_loss"]
                    ).item(),
        ...
        if "mb_rewards" in train_info:
            train_info_dict["rewards/mb_rewards"] = (
                train_info["mb_rewards"].mean().item()
            )
