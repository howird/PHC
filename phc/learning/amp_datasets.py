...
class AMPDataset(datasets.PPODataset):
    def __init__(
        ...
        super().__init__(
            batch_size, minibatch_size, is_discrete, is_rnn, device, seq_len
        )
    ...
    def update_values_dict(
        self, values_dict, rnn_format=False, horizon_length=1, num_envs=1
    ):
        ...
        if rnn_format and self.is_rnn:
            ...
            if self.values_dict["rnn_states"] is not None:
                ...
                ]  # rnn_states are not swapped in AMP, so do not swap it here.
            self._idx_buf = torch.randperm(
                self.num_envs
            )  # Update to only shuffle the envs.
    ...
    def _get_item_rnn(self, idx):
        ...
        for k, v in self.values_dict.items():
            if k not in self.special_names and v is not None:
                input_dict[k] = (
                    v[sample_idx, :].view(step_size * self.horizon_length, -1).squeeze()
                )  # flatten to batch size

        input_dict["old_values"] = input_dict["old_values"][
            :, None
        ...
        input_dict["returns"] = input_dict["returns"][
            :, None
        ]  # ZL Hack: following compute assumes that the old_values is [batch, 1], so has to change this back. Otherwise, the loss will be wrong.
