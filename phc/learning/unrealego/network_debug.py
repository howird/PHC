...
class HeatMap_UnrealEgo_AfterBackbone(nn.Module):
    def __init__(self, opt, model_name="resnet18"):
        ...
        self.conv_up3 = convrelu(
            516 * feature_scale + 1024 * feature_scale, 1024 * feature_scale, 3, 1
        )
        self.conv_up2 = convrelu(
            256 * feature_scale + 1024 * feature_scale, 512 * feature_scale, 3, 1
        )
        self.conv_up1 = convrelu(
            128 * feature_scale + 512 * feature_scale, 512 * feature_scale, 3, 1
        )
    ...
    def forward(self, list_input_left, list_input_right):
        list_stereo_feature = [
            torch.cat([list_input_left[id], list_input_right[id]], dim=1)
            for id in range(len(list_input_left))
        ]
