...
class PNN(NetworkBuilder.BaseNetwork):
    ...
    def load_actor(self, checkpoint, idx=0):
        ...
        state_dict["2.weight"].copy_(
            checkpoint["model"]["a2c_network.actor_mlp.2.weight"]
        )
        ...
        if self.has_lateral:
            ...
            if idx == 0:
                ...
                for curr_idx in range(0, idx + 1):
                    ...
                    acc_acts_1 = [
                        self.u[curr_idx - 1][col_idx][0](activation_cache[0][col_idx])
                        for col_idx in range(len(activation_cache[0]))
                    ]  # curr_idx - 1 as we need to go to the previous coloumn's index to activate the weight
                    activation_2 = curr_actor[3](
                        curr_actor[2](activation_1) + sum(acc_acts_1)
                    )  # ReLU, full
                    ...
                    actions = curr_actor[4](
                        activation_2
                    )  # disable action space transfer.
