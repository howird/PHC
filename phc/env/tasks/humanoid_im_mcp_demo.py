...
class HumanoidImMCPDemo(humanoid_im_mcp.HumanoidImMCP):
    def __init__(
        ...
        self.to_isaac_mat = torch.from_numpy(
            sRot.from_euler(
                "xyz", np.array([-np.pi / 2, 0, 0]), degrees=False
            ).as_matrix()
        ).float()
        self.to_global = torch.from_numpy(
            sRot.from_quat([0.5, 0.5, 0.5, 0.5]).inv().as_matrix()
        ).float()
        ...
        self.mean_limb_lengths = np.array(
            [0.1061, 0.3624, 0.4015, 0.1384, 0.1132], dtype=np.float32
        )[None, :]
    ...
    def _compute_task_obs_demo(self, env_ids=None):
        ...
        if self.obs_v == 6:
            ...
            filtered_trans = filters.gaussian_filter1d(
                self.root_pos_acc, 3, axis=0, mode="mirror"
            )
            ...
            _, global_rotation = humanoid_kin.forward_kinematics_batch(
                pose_mat[:, smpl_2_mujoco],
                self.zero_trans,
                self.local_translation_batch,
                self.parent_indices,
            )

            ref_rb_rot = ptr.matrix_to_quaternion_ijkr(
                global_rotation.matmul(self.to_global)
            )
            ...
            if len(self.body_rot_acc) > 0:
                ref_rb_rot_np = quat_correct_two_batch(
                    self.body_rot_acc[-1], ref_rb_rot_np
                )
                filtered_quats = filters.gaussian_filter1d(
                    np.concatenate([self.body_rot_acc, ref_rb_rot_np[None,]], axis=0),
                    1,
                    axis=0,
                    mode="mirror",
                )
                new_quat = (
                    filtered_quats[-1]
                    / np.linalg.norm(filtered_quats[-1], axis=1)[:, None]
                )
            ...
            ref_rb_pos = SkeletonState.from_rotation_and_root_translation(
                self.skeleton_trees[0], ref_rb_rot, trans, is_local=False
            ).global_translation.to(self.device)  # SLOWWWWWWW
            ref_rb_rot = ref_rb_rot.to(self.device)
            ref_rb_pos = ref_rb_pos.to(self.device)
            ref_body_ang_vel = SkeletonMotion._compute_angular_velocity(
                torch.stack([self.prev_ref_body_rot, ref_rb_rot], dim=1),
                time_delta=s_dt,
                guassian_filter=False,
            )[:, 0]
            ref_body_vel = SkeletonMotion._compute_velocity(
                torch.stack([self.prev_ref_body_pos, ref_rb_pos], dim=1),
                time_delta=s_dt,
                guassian_filter=False,
            )[:, 0]  # this is slow!
            ...
            for i in range(6):
                ...
                if parent != -1:
                    limb_lengths.append(
                        np.linalg.norm(
                            ref_rb_pos[:, parent] - ref_rb_pos[:, i], axis=-1
                        )
                    )
            ...
            filtered_root_trans[..., 2] = filters.gaussian_filter1d(
                filtered_root_trans[..., 2], 10, axis=0, mode="mirror"
            )  # More filtering on the root translation
            filtered_root_trans[..., :2] = filters.gaussian_filter1d(
                filtered_root_trans[..., :2], 5, axis=0, mode="mirror"
            )
            trans = filtered_root_trans[-1]

            self.body_pos_acc.append(ref_rb_pos)
            body_pos = np.array(self.body_pos_acc)
            filtered_ref_rb_pos = filters.gaussian_filter1d(
                body_pos, 2, axis=0, mode="mirror"
            )
