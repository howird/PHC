...
class HumanoidImDemo(humanoid_im.HumanoidIm):
            ...
            async for msg in ws:
                if msg.type == aiohttp.WSMsgType.TEXT:
                    if msg.data == "close cmd":
                        ...
                        self.j3d = (
                            torch.tensor(json_data["j3d_curr"]).to(self.device).float()
                        )
                        self.j3d_vel = (
                            torch.tensor(json_data["j3d_curr_vel"])
                            .to(self.device)
                            .float()
                        )
    ...
    def _reset_ref_state_init(self, env_ids):
        ...
        random_heading_quat = (
            torch.from_numpy(sRot.from_euler("xyz", [0, 0, np.pi]).as_quat())[None,]
            .float()
            .to(self.device)
        )
        ...
        rb_pos = quat_apply(
            random_heading_quat_repeat, rb_pos - root_pos[:, None, :]
        ).clone()
