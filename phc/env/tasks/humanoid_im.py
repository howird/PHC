...
class HumanoidIm(humanoid_amp_task.HumanoidAMPTask):
    def __init__(
        ...
        self._full_track_bodies_id = self._build_key_body_ids_tensor(
        ...
        self.random_occlu_idx = torch.zeros(
            (self.num_envs, len(self._track_bodies)),
            device=self.device,
            dtype=torch.bool,
        )
        ...
        if self.humanoid_type in [
            ...
            self.actions = torch.zeros(self.num_envs, self._dof_obs_size).to(
                self.device
            )  #### Keeping taps on previous actions

        # Overriding
        self.reward_raw = torch.zeros(
            (self.num_envs, 5 if self.power_reward else 4)
        ).to(self.device)
    ...
    def create_o3d_viewer(self):
        ...
        if self.viewer_o3d:
            ...
            box.vertex_colors = o3d.utility.Vector3dVector(
                np.array([[0.1, 0.1, 0.1]]).repeat(8, axis=0)
            )
            ...
            if self.humanoid_type in ["smpl", "smplh", "smplx"]:
                ...
                if self.humanoid_type == "smpl":
                    self.mujoco_2_smpl = [
                        self._body_names_orig.index(q)
                        for q in SMPL_BONE_ORDER_NAMES
                        if q in self._body_names_orig
                    ]
                elif self.humanoid_type in ["smplh", "smplx"]:
                    self.mujoco_2_smpl = [
                        self._body_names_orig.index(q)
                        for q in SMPLH_BONE_ORDER_NAMES
                        if q in self._body_names_orig
                    ]

                with torch.no_grad():
                    verts, joints = self._motion_lib.mesh_parsers[0].get_joints_verts(
                        pose=torch.zeros(1, len(self._body_names_orig) * 3)
                    )
            ...
            self.sim_mesh.vertex_colors = o3d.utility.Vector3dVector(
                np.array([[0, 0.5, 0.5]]).repeat(verts.shape[1], axis=0)
            )
            if self.vis_ref:
                ...
                self.ref_mesh.vertex_colors = o3d.utility.Vector3dVector(
                    np.array([[0.5, 0.0, 0.0]]).repeat(verts.shape[1], axis=0)
                )
            ...
            self.coord_trans = (
                torch.from_numpy(sRot.from_euler("xyz", [-np.pi / 2, 0, 0]).as_matrix())
                .float()
                .cuda()
            )
            ...
            self._video_path_o3d = osp.join(
                "output", "renderings", f"{self.cfg_name}-%s-o3d.mp4"
            )
        ...
        if self.viewer_o3d and self.control_i == 0:
            if self.humanoid_type in ["smpl", "smplh", "smplx"]:
                ...
                if self._has_upright_start:
                    ...
                    if (
                        self.vis_ref
                        and len(self.ref_motion_cache["dof_pos"]) == self.num_envs
                    ):
                    ...
                    pose_quat = (
                        (
                            sRot.from_quat(body_quat.reshape(-1, 4).numpy())
                            * self.pre_rot
                        )
                        .as_quat()
                        .reshape(N, -1, 4)
                    ...
                    pose_aa = torch.from_numpy(
                        pose_aa[:, self.mujoco_2_smpl, :].reshape(N, -1)
                    ).cuda()
                else:
                    ...
                    pose_aa = torch.cat(
                        [torch_utils.quat_to_exp_map(root_rot), dof_pos], dim=1
                    ).reshape(1, -1)
                    ...
                    if (
                        ...
                        ref_pose_aa = torch.cat(
                            [torch_utils.quat_to_exp_map(ref_root_rot), ref_dof_pos],
                            dim=1,
                        )
                ...
                with torch.no_grad():
                    verts, joints = self.mesh_parser.get_joints_verts(
                        pose=pose_aa, th_trans=root_trans_offset.cuda()
                    )
    ...
    def _record_states(self):
        ...
        self.state_record["ref_body_pos_subset"].append(
            self.ref_body_pos_subset.cpu().clone()
        )
    ...
    def get_task_obs_size(self):
        ...
        if self._enable_task_obs:
            if self.obs_v == 1:
                ...
                obs_size = (
                    len(self._track_bodies) * self._num_traj_samples * 24 + 30
                )  # Hard coded.
                ...
                obs_size = (
                    len(self._track_bodies) * self._num_traj_samples * 9
    ...
    def init_root_points(self):
        ...
        y = torch.tensor(
            np.linspace(-0.5, 0.5, 5), device=self.device, requires_grad=False
        )
    ...
    def _build_marker(self, env_id, env_ptr):
        ...
        for i in range(self._num_joints):
            ...
            if self.humanoid_type in ["smplx"] and self._body_names_orig[i] in [
                "L_Wrist",
                "R_Wrist",
                "L_Index1",
                "L_Index2",
    ...
    def post_physics_step(self):
        ...
        if flags.im_eval:
            ...
            self.extras["mpjpe"] = (
                (body_pos - motion_res["rg_pos"]).norm(dim=-1).mean(dim=-1)
            )
    ...
    def _compute_reward(self, actions):
        ...
        if self.zero_out_far:
            ...
            )  # For those that are outside, no imitation reward
            ...
            self.rew_buf, self.reward_raw[:, 0] = compute_point_goal_reward(
                self._point_goal, distance
            )

            im_reward, im_reward_raw = compute_imitation_reward(
                root_pos[~zeros_subset, :],
                root_rot[~zeros_subset, :],
                body_pos[~zeros_subset, :],
                body_rot[~zeros_subset, :],
                body_vel[~zeros_subset, :],
                body_ang_vel[~zeros_subset, :],
                ref_rb_pos[~zeros_subset, :],
                ref_rb_rot[~zeros_subset, :],
                ref_body_vel[~zeros_subset, :],
                ref_body_ang_vel[~zeros_subset, :],
                self.reward_specs,
            )

            # self.rew_buf, self.reward_raw = self.rew_buf * 0.5, self.reward_raw * 0.5 # Half the reward for the location reward
            self.rew_buf[~zeros_subset] = (
                self.rew_buf[~zeros_subset] + im_reward * 0.5
            )  # for those are inside, add imitation reward
            self.reward_raw[~zeros_subset, :4] = (
                self.reward_raw[~zeros_subset, :4] + im_reward_raw * 0.5
            )
            ...
            if self._full_body_reward:
                if self.humanoid_type in ["h1", "g1"]:
                    ...
                    ref_rb_pos_extend = torch.cat(
                        [ref_rb_pos, motion_res["rg_pos_t"][:, self.num_bodies :]],
                        dim=1,
                    )
                    ref_rb_rot_extend = torch.cat(
                        [ref_rb_rot, motion_res["rg_rot_t"][:, self.num_bodies :]],
                        dim=1,
                    )
    ...
    def _update_occl_training(self):
        occu_training = (
            torch.ones([self.num_envs, len(self._track_bodies)], device=self.device)
            * self._occl_training_prob
        )
        random_occlu_idx = torch.bernoulli(occu_training).bool()
        random_occlu_idx[:, 0] = False

        self.random_occlu_count[random_occlu_idx] = torch.randint(
            30, 60, self.random_occlu_count[random_occlu_idx].shape
        ).to(self.device)
        ...
        if self.cycle_motion:
            ...
            if pass_time_motion_len.sum() > 0:
                ...
                if self.cycle_motion_xp:
                    ...
                    rand_distance = (
                        torch.sqrt(torch.rand(num_cycle_motion).to(self.device))
                        * max_distance
                    )
                    rand_angle = (
                        torch.rand(num_cycle_motion).to(self.device) * np.pi * 2
                    )
                    ...
                    self._global_offset[pass_time_motion_len, :2] = (
                        ...
                        + torch.cat(
                            [
                                (torch.cos(rand_angle) * rand_distance)[:, None],
                                (torch.sin(rand_angle) * rand_distance)[:, None],
                            ],
                            dim=-1,
                        )
                    )
...
def compute_imitation_observations(
    ...
    diff_global_body_pos = ref_body_pos.view(B, time_steps, J, 3) - body_pos.view(
        B, 1, J, 3
    )
...
def compute_imitation_observations_v6(
    ...
    diff_global_body_rot = torch_utils.quat_mul(
        ref_body_rot.view(B, time_steps, J, 4),
        torch_utils.quat_conjugate(body_rot[:, None].repeat_interleave(time_steps, 1)),
    )
    diff_local_body_rot_flat = torch_utils.quat_mul(
        torch_utils.quat_mul(
            heading_inv_rot_expand.view(-1, 4), diff_global_body_rot.view(-1, 4)
...
@torch.jit.script
def compute_imitation_observations_v8(
    root_pos,
    root_rot,
    body_pos,
    body_rot,
    ...
    heading_inv_rot = torch_utils.calc_heading_quat_inv(root_rot)
    heading_rot = torch_utils.calc_heading_quat(root_rot)
    heading_inv_rot_expand = heading_inv_rot.unsqueeze(-2).repeat(
        (1, body_pos.shape[1], 1)
    )
    heading_rot_expand = heading_rot.unsqueeze(-2).repeat((1, body_pos.shape[1], 1))

    diff_global_body_pos = ref_body_pos.view(B, time_steps, J, 3)[
        :, 0:1
    ] - body_pos.view(B, 1, J, 3)
    diff_global_body_rot = torch_utils.quat_mul(
        ref_body_rot.view(B, time_steps, J, 4)[:, 0:1],
        torch_utils.quat_conjugate(body_rot).view(B, 1, J, 4),
    )
    ...
    diff_global_ang_vel = ref_body_ang_vel.view(B, time_steps, J, 3)[
        :, 0:1
    ] - body_ang_vel.view(B, 1, J, 3)
    ...
    local_ref_body_ang_vel = torch_utils.my_quat_rotate(
        heading_inv_rot_expand.view(-1, 4), ref_body_ang_vel.view(-1, 3)
    )
    ...
    if time_steps > 1:
        ...
        obs.append(
            local_ref_body_pos[:, 0].view(B, -1)
        )  # first append the current ones
        ...
        obs.append(
            local_ref_body_pos[:, 1:].reshape(B, -1)
        )  # then append the future ones
...
@torch.jit.script
def compute_imitation_observations_v9(
    root_pos,
    root_rot,
    body_pos,
    body_rot,
    ...
    ##### linear and angular  Velocity differences
    heading_inv_rot_expand_root = heading_inv_rot.unsqueeze(-1).repeat_interleave(
        time_steps, 0
    )
    ...
    diff_local_root_vel = torch_utils.my_quat_rotate(
        heading_inv_rot_expand_root.view(-1, 4), diff_global_root_vel.view(-1, 3)
    )

    diff_global_root_ang_vel = ref_body_root_ang_vel.view(
        B, time_steps, 3
    ) - root_ang_vel.view(B, 1, 3)
    diff_local_root_ang_vel = torch_utils.my_quat_rotate(
        heading_inv_rot_expand_root.view(-1, 4), diff_global_root_ang_vel.view(-1, 3)
    )
...
def compute_imitation_reward(
    ...
    reward = (
        w_pos * r_body_pos + w_rot * r_body_rot + w_vel * r_vel + w_ang_vel * r_ang_vel
    )
    ...
    if enable_early_termination:
        if use_mean:
            has_fallen = torch.any(
                torch.norm(rigid_body_pos - ref_body_pos, dim=-1).mean(
                    dim=-1, keepdim=True
                )
            ...
            has_fallen = torch.any(
                torch.norm(rigid_body_pos - ref_body_pos, dim=-1)
                > termination_distance,
                dim=-1,
            )  # using max
...
def compute_location_observations(root_pos, root_rot, target_pos, upright):
    ...
    diff_local_body_pos_flat = torch_utils.my_quat_rotate(
        heading_inv_rot, diff_global_body_pos.view(-1, 3)
    )
    max_distance = 7.5
    ...
    diff_locations[smallers] = diff_local_body_pos_flat[
        smallers, None
    ] * torch.linspace(0.2, 1, 5)[None, :, None].repeat(smallers.sum(), 1, 1).to(
        diff_local_body_pos_flat
    )  # 5 time stpes, 2 seconds
    modified_locals = (
...
def compute_humanoid_traj_reset(
    ...
    if enable_early_termination:
        ...
        body_contact_force = (
            torch.sqrt(
                torch.square(torch.abs(masked_contact_buf.sum(dim=-2))).sum(dim=-1)
            )
            > force_threshold
        )
