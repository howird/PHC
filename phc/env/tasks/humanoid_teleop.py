...
import copy
from phc.utils.lpf import ActionFilterButterTorch
...
class HumanoidTeleop(humanoid_im.HumanoidIm):
    def __init__(
        ...
        self.noise_scale_vec = torch.cat(
            [self.self_noise_vec, self.task_noise_vec], dim=-1
        )
    ...
    def _push_robots(self):
        ...
        self._humanoid_root_states[:, 7:9] = torch_rand_float(
            -max_vel, max_vel, (self.num_envs, 2), device=self.device
        )  # lin vel x/y
        self.gym.set_actor_root_state_tensor(
            self.sim, gymtorch.unwrap_tensor(self._humanoid_root_states)
        )
    ...
    def _reset_env_tensors(self, env_ids):
        ...
        if self.cfg.control.action_filter:
            ...
            filter_action_ids_torch = torch.concat(
                [
                    torch.arange(self.num_actions, dtype=torch.long, device=self.device)
    ...
    def _compute_reset(self):
        ...
        self.progress_buf[is_recovery] -= (
            1  # ZL: do not advance progress buffer for these.
        )
        return
    ...
    def _compute_reward(self, actions):
        ...
        if self.humanoid_type in [
            ...
            ref_rb_pos_extend = torch.cat(
                [ref_rb_pos, motion_res["rg_pos_t"][:, self.num_bodies :]], dim=1
            )
            ref_rb_rot_extend = torch.cat(
                [ref_rb_rot, motion_res["rg_rot_t"][:, self.num_bodies :]], dim=1
            )

            self.rew_buf[:], self.reward_raw = humanoid_im.compute_imitation_reward(
                root_pos,
                root_rot,
                body_pos_extend,
                body_rot_extend,
    ...
    def _reward_dof_pos_limits(self):
        ...
        out_of_limits = -(self._dof_pos - self.dof_pos_limits[:, 0]).clip(
            max=0.0
        )  # lower limit
        out_of_limits += (self._dof_pos - self.dof_pos_limits[:, 1]).clip(min=0.0)
        return torch.sum(out_of_limits, dim=1)

    def _reward_torque_limits(self):
        # penalize torques too close to the limit
        return torch.sum(
            (torch.abs(self.torques) - self.torque_limits * 1).clip(min=0.0), dim=1
        )
    ...
    def _reward_dof_acc(self):
        ...
        return torch.sum(
            torch.square((self.last_dof_vel - self._dof_vel) / self.dt), dim=1
        )
    ...
    def _reward_slippage(self):
        ...
        return torch.sum(
            ...
            * (torch.norm(self._contact_forces[:, self.feet_indices, :], dim=-1) > 1.0),
            dim=1,
        )
    ...
    def _reward_feet_contact_forces(self):
        ...
        return torch.sum(
            (
                torch.norm(self._contact_forces[:, self.feet_indices, :], dim=-1)
                - self.reward_specs.max_contact_force
            ).clip(min=0.0),
            dim=1,
        )
    ...
    def _reward_stumble(self):
        ...
        return torch.any(
            torch.norm(self._contact_forces[:, self.feet_indices, :2], dim=2)
            > 5 * torch.abs(self._contact_forces[:, self.feet_indices, 2]),
            dim=1,
        )
    ...
    def _reward_feet_air_time_teleop(self):
        ...
        contact = self._contact_forces[:, self.feet_indices, 2] > 1.0
        ...
        rew_airTime *= (
            torch.norm(self.ref_body_vel[:, 0, :2], dim=1) > 0.1
    ...
    def _get_noise_scale_vec(self):
        ...
        if self.self_obs_v == 4:
            ...
            # dof vel
            self_obs_noise[self.num_dof : 2 * self.num_dof] = (
                noise_scales.dof_vel * noise_level
            )
            # base vel
            self_obs_noise[2 * self.num_dof : 2 * self.num_dof + 3] = (
                noise_scales.lin_vel * noise_level
            )
            # base ang vel
            self_obs_noise[2 * self.num_dof + 3 : 2 * self.num_dof + 6] = (
                noise_scales.ang_vel * noise_level
            )
            # base gravity
            self_obs_noise[2 * self.num_dof + 6 : 2 * self.num_dof + 9] = (
