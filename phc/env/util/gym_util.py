...
def configure_gym(gym, config):
    ...
    if render:
        ...
        viewer = gym.create_viewer(
            sim,
            int(gymapi.DEFAULT_VIEWER_WIDTH / 1.25),
...
def parse_states_from_reference_states(reference_states, progress):
    ...
    global_quats_ref = torch.tensor(
        reference_states._global_rotation[(progress,)].numpy(), dtype=torch.double
    ).cuda()
    ts_ref = torch.tensor(
        reference_states._translation[(progress,)].numpy(), dtype=torch.double
    ).cuda()
    vels_ref = torch.tensor(
        reference_states._velocity[(progress,)].numpy(), dtype=torch.double
    ).cuda()
    avels_ref = torch.tensor(
        reference_states._angular_velocity[(progress,)].numpy(), dtype=torch.double
    ).cuda()
    return global_quats_ref, ts_ref, vels_ref, avels_ref


def parse_states_from_reference_states_with_motion_id(
    ...
    global_id = np.minimum(
        global_id, precomputed_state["global_quats_ref"].shape[0] - 1
    )
...
def get_flatten_ids(precomputed_state):
    ...
    for i_motion in range(len(motion_offsets) - 1):
        ...
        if len(global_id) == 0:
            ...
            global_id.extend(range(global_id[-1] + 1, global_id[-1] + i_length + 1))
    return np.array(init_state_id), np.array(init_motion_id), np.array(global_id)
...
def get_robot_states_from_torch_tensor(
    config,
    ts,
    global_quats,
    vels,
    ...
    if config["env_orientation_ob"]:
        if relative_rot is not None:
            ...
            relative_rot = (
                quaternion_math.compute_orientation_drift(init_rot, curr_rot)
                .cpu()
                .numpy()
            )
            obs = np.concatenate([obs, relative_rot], axis=-1)
    ...
    if config["env_frame_ob"]:
        if type(motion_length) == np.ndarray:
            ...
            progress_ob = np.expand_dims(
                progress.astype(float) / motion_length, axis=-1
            )
        else:
            progress_ob = np.expand_dims(
                progress.astype(float) / float(motion_length), axis=-1
            )
        obs = np.concatenate([obs, progress_ob], axis=-1)

    if config["env_motion_ob"] and not config["env_motion_ob_onehot"]:
        motion_id_ob = np.expand_dims(
            motion_id.astype(float) / float(num_motion), axis=-1
        )
